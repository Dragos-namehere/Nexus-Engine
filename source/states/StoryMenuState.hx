...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		scoreText = new FlxText(-500, 10, 0, Language.getPhrase('week_score', 'WEEK SCORE: {1}', [lerpScore]), 36);
		//scoreText = new FlxText(10, 10, 0, Language.getPhrase('week_score', 'WEEK SCORE: {1}', [lerpScore]), 36);
		scoreText.setFormat(Paths.font("vcr.ttf"), 32);

		FlxTween.tween(scoreText, {x : 10 }, 0.5, {ease : FlxEase.backOut, startDelay: 0.5});
			...
			if(!isLocked || !weekFile.hiddenUntilUnlocked)
			{
				...
				if (isLocked)
				{
				  //var lock:FlxSprite = new FlxSprite(weekThing.width + 10 + weekThing.x);
					var lock:FlxSprite = new FlxSprite(1200);
					...
					lock.ID = i;
					grpLocks.add(lock);
					FlxTween.tween(lock, {x : weekThing.width + 10 + weekThing.x }, 0.5, {ease : FlxEase.backOut, startDelay: 0.3});
				}
				num++;
			}
		}
