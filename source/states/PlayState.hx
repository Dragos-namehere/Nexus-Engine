...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		ClientPrefs.data.timeBarType = 'Song Name';
		...
		//healthBar = new Bar(0, FlxG.height * 0.89, 'healthBar', function() return health, 0, 2); (this is the for the 3d one, next one is smoother and 3d)
		healthBar = new Bar(0, FlxG.height * 0.89, 'healthBar', function() return displayedHealth, 0, 2);
		healthBar.screenCenter(X);
		healthBar.leftToRight = false;
		//healthBar.scrollFactor.set(0.25, 0);
		healthBar.visible = !ClientPrefs.data.hideHud;
		//healthBar.alpha = ClientPrefs.data.healthBarAlpha;
		healthBar.alpha = 0;
		FlxTween.tween(healthBar, {alpha: ClientPrefs.data.healthBarAlpha}, 1, {ease: FlxEase.sineInOut, startDelay: 0.5});
		...
		iconP1.alpha = 0;
		FlxTween.tween(iconP1, {alpha: ClientPrefs.data.healthBarAlpha}, 1, {ease: FlxEase.sineInOut, startDelay: 0.5});
		healthBarGroup.add(iconP1);
		...
		iconP2.alpha = 0;
		FlxTween.tween(iconP2, {alpha: ClientPrefs.data.healthBarAlpha}, 1, {ease: FlxEase.sineInOut, startDelay: 0.5});
		healthBarGroup.add(iconP2);
                          //-300
		...
		sickCounter = new FlxText(0, FlxG.height - 200., FlxG.width, "", 20);
		sickCounter.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		sickCounter.screenCenter(X);
		sickCounter.scrollFactor.set();
		sickCounter.borderSize = 1.25;
		sickCounter.visible = !ClientPrefs.data.hideHud;
		uiGroup.add(sickCounter);

		goodCounter = new FlxText(0, FlxG.height - 170, FlxG.width, "", 20);
		goodCounter.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		goodCounter.screenCenter(X);
		goodCounter.scrollFactor.set();
		goodCounter.borderSize = 1.25;
		goodCounter.visible = !ClientPrefs.data.hideHud;
		uiGroup.add(goodCounter);

		badCounter = new FlxText(0, FlxG.height - 140., FlxG.width, "", 20);
		badCounter.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		badCounter.screenCenter(X);
		badCounter.scrollFactor.set();
		badCounter.borderSize = 1.25;
		badCounter.visible = !ClientPrefs.data.hideHud;
		uiGroup.add(badCounter);

		shitCounter = new FlxText(0, FlxG.height - 110., FlxG.width, "", 20);
		shitCounter.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		shitCounter.screenCenter(X);
		shitCounter.scrollFactor.set();
		shitCounter.borderSize = 1.25;
		shitCounter.visible = !ClientPrefs.data.hideHud;
		uiGroup.add(shitCounter);
		...
		healthBarGroup.cameras = [camGame];
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			FlxTween.tween(babyArrow, {y : strumLineY}, 1, {ease : FlxEase.backOut, startDelay: 0.3 + idd*0.1});
			idd++;
	...
	displayedHealth = FlxMath.lerp(displayedHealth, health, elapsed * playbackRate);

    //The Counter thingy
	var sickscount = ratingsData[0].hits;
	var goodscount = ratingsData[1].hits;
	var badcount = ratingsData[2].hits;
	var shitscount = ratingsData[3].hits;

	sickCounter.text = 'Sicks: $sickscount';
	goodCounter.text = 'Goods: $goodscount';
	badCounter.text = 'Bads: $badcount';
	shitCounter.text = 'Shits: $shitscount';
	...
	public dynamic function updateIconsPosition(elapsed:Float)
	{
		...
		//FlxTween.tween(iconP1, {x : healthBar.barCenter + (150 * iconP1.scale.x - 150) / 2 - iconOffset}, 0.1, {ease : FlxEase.quadIn});

		iconP2.x = healthBar.barCenter - (150 * iconP2.scale.x) / 2 - iconOffset * 2;

		//FlxTween.tween(iconP2, {x : healthBar.barCenter - (150 * iconP2.scale.x) / 2 - iconOffset * 2}, 0.1, {ease : FlxEase.quadIn});
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if(ret != LuaUtils.Function_Stop)
			{
				...
				//FlxG.camera.setFilters([]); //setFilters is deprecated, I used an filters array this time
				var filtersArray = [];
				FlxG.camera.filters = filtersArray;
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
	  ...
	  case 'Resize Window':
        ...
        var windowSize = {w: Application.current.window.width, h: Application.current.window.height};

		FlxTween.tween(windowSize, {w: flValue1, h: flValue2}, 0.5, {ease: FlxEase.quadInOut,
            onUpdate: function(tween) {
                // Update the window's width and height during the tween
                Application.current.window.width = Std.int(windowSize.w);
	...
	// Stores HealthBar Objects in a Group
	...
	override function beatHit()
	{
		...
		FlxTween.tween(iconP1, { angle: 2 }, Conductor.crochet/1000, {ease : FlxEase.circOut});
		FlxTween.tween(iconP2, { angle: 2 }, Conductor.crochet/1000, {ease : FlxEase.circOut});
