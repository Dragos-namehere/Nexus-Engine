			...
			case 'noteSkin', 'splashSkin', 'splashAlpha':
				if(!notesShown)
				{
					...
					for (note in notes.members)
					{
						...
						FlxTween.tween(note, {y : noteY }, 0.5, {ease : FlxEase.backOut, startDelay: 0.5 + idd*0.1, onComplete: function(tween:FlxTween){
							trace("arrow animrtt doner");
						}});
						idd++;
					}
				}
			...
			default:
				if(notesShown) 
				{
					...
					for (note in notes.members)
					{
						...
						FlxTween.tween(note, {y : -200 }, 0.5, {ease : FlxEase.backIn, startDelay: 0.5 + idd*0.1, onComplete: function(tween:FlxTween){
							trace("arrowanimar doner");
						}});
						idd++;
					}
				}
